...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case spinner.TickMsg:
		if m.claudeRunning || m.executing {
			var cmd tea.Cmd
...
func (m Model) handleDashboardKey(msg tea.KeyMsg) (tea.Model, tea.Cmd) {
	...
	case "R":
		if !m.executing {
			n := m.resetFeatures(true)
			if n > 0 {
				m.workerLogs = append(m.workerLogs, fmt.Sprintf("[ORCH] Reset ALL %d features to pending", n))
			}
			m.updateDashboardContent()
			return m, nil
		}
...
func (m Model) generateMissionFromSpec(spec SpecEntry) (tea.Model, tea.Cmd) {
	...
	existingAnalysis := readFileContent(filepath.Join(spec.SpecPath, "mission", "codebase-analysis.md"))
	...
	m.discoveryMsgs = []ChatMessage{
		...
		{Role: "system", Text: "Estimated time: 10-20 minutes across 4 sequential phases."},
		{Role: "system", Text: ""},
	...
	return m, tea.Batch(
		m.spinner.Tick,
		func() tea.Msg {
			prompt := BuildAnalysisPrompt(specPath, projectDir, existingAnalysis)
...
func (m Model) nextGenPhase(result string) (tea.Model, tea.Cmd) {
	...
	case GenPhaseAnalysis:
		...
		return m, func() tea.Msg {
			prompt := BuildAssertionPrompt(specPath)
			ch := make(chan ClaudeStreamMsg, 64)
			v := verboseVal
			args := []string{"--allowedTools", "Read", "--no-verbose", "--max-turns", "5"}
			cmd := StartClaude(prompt, projectDir, &v, ch, args...)
			return contextReadyMsg{ch: ch, cmd: cmd, prompt: prompt, extraArgs: args}
		}

	case GenPhaseAssertions:
		assertions := ParseAssertionsJSON(result)
		if len(assertions) == 0 {
			return m.retryGenPhase("Could not parse assertions from output")
		}
		...
		m.discoveryMsgs = append(m.discoveryMsgs, ChatMessage{Role: "system", Text: fmt.Sprintf("PHASE_DONE:2:%s:%d assertions across %d categories", elapsed, total, len(assertions))})
		...
		return m, func() tea.Msg {
			prompt := BuildFeaturesOnlyPrompt(specPath)
			ch := make(chan ClaudeStreamMsg, 64)
			v := verboseVal
			args := []string{"--allowedTools", "Read", "--no-verbose"}
			cmd := StartClaude(prompt, projectDir, &v, ch, args...)
			return contextReadyMsg{ch: ch, cmd: cmd, prompt: prompt, extraArgs: args}
		}

	case GenPhaseFeatures:
		features := ParseFeaturesJSON(result)
		if len(features) == 0 {
			plan := ParsePlanFromText(result)
			if plan != nil && len(plan.Features) > 0 {
				features = plan.Features
			}
		}
		if len(features) == 0 {
			return m.retryGenPhase("Could not parse features from output")
		}
		m.pendingFeatures = features
		m.discoveryMsgs = append(m.discoveryMsgs, ChatMessage{Role: "system", Text: fmt.Sprintf("PHASE_DONE:3:%s:%d features decomposed", elapsed, len(features))})
		...
		return m, func() tea.Msg {
			prompt := BuildKnowledgePrompt(specPath)
			ch := make(chan ClaudeStreamMsg, 64)
			v := verboseVal
			args := []string{"--allowedTools", "Read", "--no-verbose"}
			cmd := StartClaude(prompt, projectDir, &v, ch, args...)
	...
	case GenPhaseKnowledge:
		...
		knowledgeDetail := fmt.Sprintf("%d entries extracted", len(knowledge))
		m.discoveryMsgs = append(m.discoveryMsgs, ChatMessage{Role: "system", Text: fmt.Sprintf("PHASE_DONE:4:%s:%s", elapsed, knowledgeDetail)})
		...
		totalTime := time.Since(m.genStartTime).Round(time.Second)
		m.discoveryMsgs = append(m.discoveryMsgs, ChatMessage{Role: "system", Text: fmt.Sprintf("GEN_COMPLETE:%s:%d features, %d assertions", totalTime, len(features), totalAssertions)})
		m.viewport.SetContent(m.renderChatContent())
		m.viewport.GotoBottom()
		return m, sendNotification("Mission", fmt.Sprintf("Plan complete — %d features, %d assertions", len(features), totalAssertions))
	}
...
func (m Model) retryClaude() (tea.Model, tea.Cmd) {
	...
	if m.claudeSessionID != "" && hadToolCalls {
...
func (m Model) handleWorkerEvent(ev WorkerEvent) (tea.Model, tea.Cmd) {
	...
	if ev.AllDone {
		...
		return m, sendNotification("Mission", fmt.Sprintf("Execution complete — %d/%d done, %d blocked", stats.Done, stats.Total, stats.Blocked))
	}
...
var phaseInfo = [4]struct{ model, desc string }{
	{"sonnet", "Codebase analysis"},
	{"opus", "Assertion derivation"},
	{"opus", "Feature decomposition"},
	{"opus", "Knowledge extraction"},
}
...
func (m Model) renderSystemMsg(text string) string {
	...
	if strings.HasPrefix(text, "PHASE_START:") {
		...
		if idx >= 1 && idx <= 4 {
			p := phaseInfo[idx-1]
			return "\n" +
				m.styles.Blue.Render(fmt.Sprintf("  ▶ Phase %d/4", idx)) + " " +
	...
	if strings.HasPrefix(text, "PHASE_DONE:") {
		...
		if len(parts) == 4 {
			...
			if idx >= 1 && idx <= 4 {
				p := phaseInfo[idx-1]
				return m.styles.Green.Render(fmt.Sprintf("  ✓ Phase %d/4", idx)) + " " +
					m.styles.Magenta.Render(p.model) + " " +
					m.styles.Dim.Render("— "+p.desc) + " " +
...
func (m Model) reviewView() string {
	...
	hint := m.styles.Hint.Render("  Enter: approve · Tab: switch · Shift+Enter: newline · Ctrl+E: editor · Esc: reject")
...
func (m Model) renderStaticOverview(totalW, leftW, rightW, leftInner, rightInner int, border lipgloss.Border, borderColor lipgloss.Color, phaseNames []string) string {
	...
	for i, f := range m.mission.Features {
		...
		if i == m.featureCursor {
			right.WriteString(fmt.Sprintf("%s%s %s %s\n", cursor, style.Render(icon), style.Render(f.ID), style.Render(title)))
		} else {
			right.WriteString(fmt.Sprintf("%s%s %s %s\n", cursor, style.Render(icon), f.ID, title))
		}
	}
...
func (m Model) renderKanbanTab() string {
	...
	for _, col := range cols {
		...
		for _, f := range col.features {
			...
			if selected {
				content.WriteString(fmt.Sprintf("%s%s %s %s\n", prefix, st.Render(icon), st.Render(f.ID), st.Render(title)))
			} else {
				content.WriteString(fmt.Sprintf("%s%s %s %s\n", prefix, st.Render(icon), f.ID, title))
			}
		}
...
func (m Model) dashboardView() string {
	...
	if m.executing {
		...
		if m.mission.Stats.Done > 0 {
			parts = append(parts, "R: reset all")
		}
